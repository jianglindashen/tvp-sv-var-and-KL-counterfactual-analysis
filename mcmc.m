...
function mcmc(nsim,datetime,timeformat)
...
npmt = 6;                   % # of parameter to store %增加sa2
...
aspar = char('sb1  ', 'sb2', 'sa1','sa2', 'sh1', 'sh2'); %增加sa2
aspar2 = char('  s_{b1}', '  s_{b2}', '  s_{a1}', '  s_{a2}', ... %增加sa2
              '  s_{h1}', '  s_{h2}');
...
for i = 1 : nk
    subplot(2, nk, i);
    plot(datetime(nl+1:end,1),m_my(nl+1:end, i))
    ...
    xlim([tstart tend]);
    if yl(1) * yl(2) < 0
        line([tstart, tend], [0, 0], 'Color', ones(1, 3)*0.6)
    end
    if i == 1
      title(['Data: ', char(m_asvar(i))],'interpreter', 'latex')
    else
...
for i = 1 : nk
    subplot(2, nk, i+nk);
    plot(datetime(nl+1:end,1),exp(msamph(nl+1:end, i)))
    datetick('x', timeformat);
    hold on
    plot(datetime(nl+1:end,1),exp(msamph(nl+1:end, i) - msamphs(nl+1:end, i)), 'r:')
    datetick('x', timeformat);
    plot(datetime(nl+1:end,1),exp(msamph(nl+1:end, i) + msamphs(nl+1:end, i)), 'r:')
    datetick('x', timeformat);
    hold off
    ...
    xlim([tstart tend]);
    if i == 1
      title(['SV $\sigma_t^2=\exp(h_t)$: ',char(m_asvar(i))], 'interpreter', 'latex')
...
xlswrite('tvpvar_vol.xlsx', [(1:ns)', mout], 'Sheet1', 'A3');
...
for i = 1 : na
    ...
    plot(datetime(nl+1:end,1),msampa(nl+1:end, i))
    datetick('x', timeformat);
    hold on
    plot(datetime(nl+1:end,1),msampa(nl+1:end, i) - msampas(nl+1:end, i), 'r:')
    datetick('x', timeformat);
    plot(datetime(nl+1:end,1),msampa(nl+1:end, i) + msampas(nl+1:end, i), 'r:')
    datetick('x', timeformat);
    hold off
    ...
    title(['$a_{', num2str(i), 't}$'],'interpreter', 'latex')
    if i == 1
      legend('Posterior mean', '1SD bands')
    end
end
...
xlswrite('tvpvar_a.xlsx', [(1:ns)', mout], 'Sheet1', 'A3');
...
for i = 1 : na
    subplot(ceil(na/2), 2, i);
    plot(datetime(nl+1:end,1),msampai(nl+1:end, i))
    datetick('x', timeformat);
    hold on
    plot(datetime(nl+1:end,1),msampai(nl+1:end, i) - msampais(nl+1:end, i), 'r:')
    datetick('x', timeformat);
    plot(datetime(nl+1:end,1),msampai(nl+1:end, i) + msampais(nl+1:end, i), 'r:')
    datetick('x', timeformat);
    hold off
...
xlswrite('tvpvar_ai.xlsx', [(1:ns)', mout], 'Sheet1', 'A3');

if isequal(exist('tvpvar_b.xlsx', 'file'), 2)
  delete('tvpvar_b.xlsx');
end
xlswrite('tvpvar_b.xlsx', [msampb/nsim], 'Sheet1', 'A1');
if m_fli == 1
  ...
  for i = 1 : nk
    ...
    plot(datetime(nl+1:end,1),msampi(nl+1:end, i))
    datetick('x', timeformat);
    hold on
    plot(datetime(nl+1:end,1),msampi(nl+1:end, i) - msampis(nl+1:end, i), 'r:')
    datetick('x', timeformat);
    plot(datetime(nl+1:end,1),msampi(nl+1:end, i) + msampis(nl+1:end, i), 'r:')
    datetick('x', timeformat);
    hold off
  ...
  xlswrite('tvpvar_int.xlsx', [(1:ns)', mout], 'Sheet1', 'A3');
end
